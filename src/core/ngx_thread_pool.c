...
ngx_thread_pool_add(ngx_conf_t *cf, ngx_str_t *name)
{
    ...
    tp->name = *name;// 线程池名字
    tp->file = cf->conf_file->file.name.data;// 定义线程池的配置文件
    tp->line = cf->conf_file->line; // 定义线程池指令的行号
