...
/**
 * 详情请参考 https://github.com/vislee/leevis.com/issues/106
报文头：
说明：
...
ngx_resolve_start(ngx_resolver_t *r, ngx_resolver_ctx_t *temp)
{
    ...
    //如果在proxy_pass里面包含变量，而且没有配置resolver，那将会返回错误了
    //不存在, 返回NGX_NO_RESOLVER, 最终请求返回502
...
ngx_resolve_name_done(ngx_resolver_ctx_t *ctx)
{
    ...
    //调用ngx_resolver_expire, 遍历r->name_expire_queue队列删除r->name_rbtree中的过期r
    if (ctx->service.len) {
...
//遍历r->name_expire_queue队列删除r->name_rbtree中的过期rn
static void
...
//超时事件的handler  重传函数
//遍历r->name_resend_queue链表, 如果存在rn->waiting, 表示还有请求在等待域名解析结果, 需要重新发送域名
static void
...
/**
当DNS服务器响应时，会调用ngx_resolver_read_response函数，
 ngx_resolver_read_response会接收数据然后调用ngx_resolver_process_response来处理响应。
ngx_resolver_process_response则会根据响应类别调用ngx_resolver_process_a或ngx_resolver_process_ptr，
 在ngx_resolver_process_a与ngx_resolver_process_ptr中则会将DNS缓存并更新有效期，调用最后的回调函数
 */
static void
