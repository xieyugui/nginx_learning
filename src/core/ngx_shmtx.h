...
* @desc:
 * https://github.com/xieyugui/reading-code-of-nginx-1.9.2/blob/master/nginx-1.9.2/src/core/ngx_shmtx.h
*/
...
#ifndef NGINX_LEARNING_NGX_SHMTX_H
#define NGINX_LEARNING_NGX_SHMTX_H
...
//开启的话，ngx_shmtx_t预处理命令后的结构包含信号量相关变量sem_t类型的sem
...
typedef struct {
//支持原子操作的情况，也就是由NGX_HAVE_ATOMIC_OPS这个宏来进行控制
#if (NGX_HAVE_ATOMIC_OPS)
    ngx_atomic_t  *lock;
#if (NGX_HAVE_POSIX_SEM)
    //如果lock锁原先的值为0，也就是说，并没有让某个进程持有锁，这时直接返回；或者，semaphore标志位为0，表示不需要使用信号量，也立即返回
    ngx_atomic_t  *wait;
    ngx_uint_t     semaphore; //信号量的值，这个值大于0表示该信号量可用，默认为1，  semaphore为1时表示获取锁将可能使用到的信号量
    sem_t          sem; // sem就是信号量锁
#endif
#else
    ...
    由于原子变量实现的这5种互斥锁方法是Nginx中使用最广泛的同步方式，当需要Nginx支持数以万计的并发TCP请求时，通常都会把spin值设为(ngx_uint_t)-1。
...
#endif //NGINX_LEARNING_NGX_SHMTX_H
